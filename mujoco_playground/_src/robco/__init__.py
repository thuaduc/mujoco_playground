...
_randomizer = {
    # No domain randomizers implemented yet
}
...
def load(
  ...
  if env_name not in _envs:
    raise ValueError(
        f"Env '{env_name}' not found. Available envs: {list(_envs.keys())}"
    )
...
def get_domain_randomizer(
  ...
  if env_name not in _randomizer:
    print(
        f"Env '{env_name}' does not have a domain randomizer in the robco"
        " registry."
    )
    return None
