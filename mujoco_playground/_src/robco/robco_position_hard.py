...
class RobcoPositionHard(RobcoArmBase):
  ...
  def __init__(
    ...
    self._target_mocap_id = self._mj_model.body(self.target_body_name).mocapid[0]
  ...
  def reset(self, rng: jax.Array, target_pos: Optional[jax.Array] = None) -> mjx_env.State:
    """Reset the environment and return initial State (mjx Data + obs, metrics...).
    
    Args:
        rng: Random number generator key.
        target_pos: Optional fixed target position for evaluation. If None, randomizes target.
    """
    ...
    # Use provided target position or randomize target position in spherical shell
    if target_pos is None:
        rng, rng_target = jax.random.split(rng)
        
        # Sample random direction (uniform on sphere)
        rng_direction, rng_radius = jax.random.split(rng_target)
        direction = jax.random.normal(rng_direction, (3,))
        direction = direction / jp.sqrt(jp.sum(direction**2))
        
        # Sample radius uniformly in [min_norm, max_norm]
        radius = jax.random.uniform(
            rng_radius,
            minval=self._config.target_min_norm,
            maxval=self._config.target_max_norm,
        )
        
        target_pos = direction * radius
    
    # Update mocap body position and orientation
    data = data.replace(
        mocap_pos=data.mocap_pos.at[self._target_mocap_id].set(target_pos),
        mocap_quat=data.mocap_quat.at[self._target_mocap_id].set(jp.array([1.0, 0.0, 0.0, 0.0]))
    )
    
    # Update xpos directly for mocap body to ensure it's visible
    data = data.replace(
        xpos=data.xpos.at[self._target_body_id].set(target_pos)
    )
